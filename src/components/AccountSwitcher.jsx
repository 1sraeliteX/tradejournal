...
export default function AccountSwitcher({ accounts, selectedId, onSelect, onAccountsChange }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <span>{selected ? selected.name : 'All Accounts'}</span>
        ...
        <div className="absolute top-full left-0 mt-1 bg-neutral-800 border border-neutral-700 rounded-lg py-1 min-w-[200px] shadow-lg z-50">
          ...
          {!showForm ? (
            <button
              ...
              className="w-full text-left px-4 py-2 text-sm text-emerald-400 hover:text-emerald-300 transition-colors flex items-center gap-1.5"
            >
            ...
            <div className="px-3 py-2 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs text-neutral-400">New Account</span>
              ...
              <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)}
                ...
                className="w-full bg-neutral-900 border border-neutral-700 rounded px-2 py-1.5 text-xs text-white focus:outline-none focus:border-emerald-500" />
              <input type="number" step="0.01" min="0" value={formCapital} onChange={(e) => setFormCapital(e.target.value)}
                placeholder="Capital ($)"
                className="w-full bg-neutral-900 border border-neutral-700 rounded px-2 py-1.5 text-xs text-white focus:outline-none focus:border-emerald-500" />
              <button onClick={handleCreate} disabled={creating}
                className="w-full bg-emerald-600 hover:bg-emerald-500 text-white rounded py-1.5 text-xs font-medium transition-colors disabled:opacity-50">
